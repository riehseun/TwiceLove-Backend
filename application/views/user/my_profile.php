...
<body>
	...
	<script>
		$(function() {
            var profile = "";
            $.getJSON('<?= base_url(); ?>user/get_my_profile', function(response, text, jqZHR) {    
                var id = response["id"];
                ...
                profile += '</div>';
                /*
                profile += '<div style="clear:both; margin-top:5%;">';
                profile += '<form method="post" action="/efa/user/give_coin" style="display:inline-block; width:30%; margin:1%;">';
                profile += '<input name="coin" type="submit" class="btn3d btn btn-gray" value="<?php echo $c[$i]; ?> Coin" style="width:100%;">';
                profile += '<input type="hidden" name="eid" value="<?php echo $emp_id; ?>">';
                profile += '<input type="hidden" name="quantity" value="<?php echo $c[$i]; ?>">';
                profile += '<input type="hidden" name="kudo" value="1">';
                profile += '</form>';
                profile += '</div>';
                */
                profile += '<hr>';
